  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-2">
        {data?.data.accounts.filter((account: any) => account.name.includes('RRSP') || account.name.includes('TFSA')).map((account: any) => <AccountProgress name={account.name} progress={0.4} key={account.id} />)}
      </div>
    </div>
  );
}
